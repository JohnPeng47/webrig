(function () {
  ...
  let staticIndicatorEl: HTMLDivElement | null = null;

  let agentActive = false;
  let staticActive = false;

  // State preserved across HIDE_FOR_TOOL_USE / SHOW_AFTER_TOOL_USE
  let wasAgentActive = false;
  let wasStaticActive = false;
  ...
  function createStaticIndicator(): HTMLDivElement {
    const el = document.createElement('div');
    el.id = 'claude-static-indicator-container';
    ...
    const chatBtn = el.querySelector('#claude-static-chat-button') as HTMLButtonElement | null;
    const chatTooltip = el.querySelector('#claude-static-chat-tooltip') as HTMLSpanElement | null;
    if (chatBtn) {
    ...
    const closeBtn = el.querySelector('#claude-static-close-button') as HTMLButtonElement | null;
    const closeTooltip = el.querySelector('#claude-static-close-tooltip') as HTMLSpanElement | null;
    if (closeBtn) {
  ...
  function showAgentIndicators(): void {
    ...
    // Show or create glow border
    if (glowEl) {
      glowEl.style.display = '';
    } else {
      glowEl = createGlowBorder();
      document.body.appendChild(glowEl);
    }

    // Show or create stop button (skip in MCP mode)
    if (!isMcpMode) {
      if (stopContainerEl) {
    ...
    requestAnimationFrame(() => {
      ...
      if (stopContainerEl) {
        const btn = stopContainerEl.querySelector(
  ...
  chrome.runtime.onMessage.addListener(
    (
      message: { type: string; action?: string; text?: string; isMcp?: boolean },
      ...
      if (message.action === 'toast') {
        const el = document.createElement('div');
        el.textContent = message.text || 'Extension updated';
        el.style.cssText = `
      ...
      // ── Reload confirmation banner ──────────────────────────────
      if (message.action === 'reload_confirm') {
        // Remove any existing reload banner first
        document.getElementById('claude-reload-banner')?.remove();

        const banner = document.createElement('div');
        banner.id = 'claude-reload-banner';
        ...
        label.textContent = message.text || 'Extension reload ready';
        ...
        `;
        reloadBtn.addEventListener('mouseenter', () => { reloadBtn.style.background = '#c56545'; });
        reloadBtn.addEventListener('mouseleave', () => { reloadBtn.style.background = '#D97757'; });
        reloadBtn.addEventListener('click', async () => {
          try {
            await chrome.runtime.sendMessage({ type: 'CONFIRM_EXTENSION_RELOAD' });
        ...
        dismissBtn.style.cssText = `
          padding: 6px 14px; background: transparent; color: #aaa; border: 1px solid #444;
          border-radius: 6px; font-size: 13px; cursor: pointer;
          font-family: inherit; transition: background 0.15s, color 0.15s;
        `;
        dismissBtn.addEventListener('mouseenter', () => { dismissBtn.style.background = '#333'; dismissBtn.style.color = '#fff'; });
        dismissBtn.addEventListener('mouseleave', () => { dismissBtn.style.background = 'transparent'; dismissBtn.style.color = '#aaa'; });
        dismissBtn.addEventListener('click', () => { banner.style.opacity = '0'; banner.style.transform = 'translateY(-8px)'; setTimeout(() => banner.remove(), 300); });
      ...
      if (message.type === 'SHOW_AGENT_INDICATORS') {
        ...
        // Temporarily hide everything for screenshots
        wasAgentActive = agentActive;
        wasStaticActive = staticActive;

        if (glowEl) glowEl.style.display = 'none';
        if (stopContainerEl) stopContainerEl.style.display = 'none';
        if (staticIndicatorEl && staticActive) staticIndicatorEl.style.display = 'none';
        ...
        if (wasStaticActive && staticIndicatorEl) {
          staticIndicatorEl.style.display = '';
        }
        wasAgentActive = false;
        wasStaticActive = false;
